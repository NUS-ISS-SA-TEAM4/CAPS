...
public class ScheduleArrangement {
    ...
    public Boolean generate()
    {
        ...
        for (var courseLecturer:courseLecturers) {
            //Lecturer lecturer=courseLecturer.getLecturer();
            //Course course=courseLecturer.getCourse();
        ...
        for(int i=0;i< day;i++)
        {
            for(int j=0;j< classrooms.size();j++) {
                ...
                courseScheduleService.createCourseSchedule(new CourseSchedule(lecturers[i][j].getCourse(),schedules[i][j],classrooms.get(j)));
            }
        }
        return true;
    }
